metadataTool.repo("MetadataRepo", function MetadataRepo(Metadata, WsApi) {
    ...
    this.get = function (document) {
        angular.extend(this.mapping.instantiate, {
            'method': 'get/' + document.name
        });
        return new Metadata();
    };

    this.unlockProject = function (project) {
        angular.extend(this.mapping.unlock, {
    ...
    this.export = function (project, format) {
        angular.extend(this.mapping.export, {
    ...
    this.getHeaders = function (format, project) {
        angular.extend(this.mapping.headers, {
            'method': 'headers/' + format + "/" + project
        });
